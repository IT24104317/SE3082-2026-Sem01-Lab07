...
int main(int argc, char **argv) {
    ...
    int chunk_size = N / size, *array = NULL;
    ...
    if (local_chunk == NULL || (rank == 0 && (array == NULL || all_sums == NULL))) MPI_Abort(MPI_COMM_WORLD, 1);
    ...
    long long local_sum = 0; for (int i = 0; i < chunk_size; ++i) local_sum += local_chunk[i];
    MPI_Gather(&local_sum, 1, MPI_LONG_LONG, all_sums, 1, MPI_LONG_LONG, 0, MPI_COMM_WORLD);
    if (rank == 0) { long long total_sum = 0; for (int r = 0; r < size; ++r) total_sum += all_sums[r]; long long expected = (long long)N * (N + 1) / 2; printf("[Scatter + Gather] Total = %lld\nExpected = %lld | Correct? %s | Time = %.6f sec\n", total_sum, expected, total_sum == expected ? "YES" : "NO", MPI_Wtime() - start); }
    free(array); free(local_chunk); free(all_sums); MPI_Finalize(); return 0;
}
